...
const App: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"GUIDE" | "ROADMAP" | "INTERVIEW">(
    "GUIDE"
  );
    ...
    return (
      <div className="min-h-screen bg-[#050810] flex items-center justify-center p-6">
        <div className="glass-card max-w-md w-full p-10 rounded-[2.5rem] text-center space-y-8 shadow-2xl">
          <div className="w-16 h-16 bg-indigo-600 rounded-2xl flex items-center justify-center font-black text-2xl mx-auto rotate-3 text-white italic shadow-lg shadow-indigo-600/50">
            P
          </div>
          ...
          <p className="text-slate-400 text-sm leading-relaxed">
            Initialize your AI engine by linking a project to unlock the career
            architect.
          </p>
          <div className="space-y-4 pt-4">
            <button
              onClick={handleSelectKey}
              className="w-full py-5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-black uppercase text-xs tracking-widest transition-all active:scale-95 shadow-xl"
            >
              Connect Project
            </button>
            <a
              ...
              target="_blank"
              className="block text-[10px] text-slate-600 hover:text-indigo-400 uppercase tracking-widest transition-colors"
            >
    ...
    <div className="min-h-screen bg-[#02040a] text-slate-200">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-3 lg:sticky lg:top-32 space-y-4 h-fit">
                  ...
                  <div className="hidden lg:block p-6 bg-slate-900/30 border border-slate-800/40 rounded-3xl space-y-4">
                    <div>
                      <h4 className="text-[9px] font-black uppercase text-slate-600 mb-1">
                        Expectation
                      </h4>
                      <div className="text-lg font-black text-white italic">
                        {guide.salaryExpectation}
                      </div>
                    </div>
                    <div>
                      <h4 className="text-[9px] font-black uppercase text-slate-600 mb-1">
                ...
                <div className="lg:col-span-6 space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-8 bg-emerald-500/5 border border-emerald-500/10 rounded-3xl">
                      <h4 className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-6">
                        Strengths
                      </h4>
                      <ul className="space-y-4">
                        {guide.prosAndCons.pros.map((p, i) => (
                          <li
                            key={i}
                            className="text-sm text-slate-400 flex gap-3 italic"
                          >
                    ...
                    <div className="p-8 bg-rose-500/5 border border-rose-500/10 rounded-3xl">
                      <h4 className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-6">
                        Barriers
                      </h4>
                      <ul className="space-y-4">
                ...
                <div className="lg:col-span-3 space-y-6">
                  <h3 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] px-2">
                    Reading Deck
                  </h3>
                  <div className="grid grid-cols-1 gap-4">
                    {guide.essentialReading.map((book, i) => (
                      <a
                        ...
                        href={book.link}
                        target="_blank"
                        className="p-6 bg-slate-900/30 border border-slate-800/40 rounded-2xl hover:border-indigo-500/50 transition-all group"
                      >
                        <div className="text-xs font-black text-white group-hover:text-indigo-400 transition-colors uppercase italic mb-1">
                          {book.title}
                        </div>
                        <p className="text-[9px] text-slate-600 font-bold uppercase mb-3">
      ...
      {error && (
        <div className="max-w-md mx-auto p-12 bg-rose-950/20 border border-rose-500/20 rounded-[2rem] text-center space-y-6">
          ...
          <h3 className="text-xl font-black uppercase italic text-rose-500">
            Connection Failed
          </h3>
          <p className="text-slate-400 text-sm">{error}</p>
          <button
            onClick={() => window.location.reload()}
            className="px-10 py-4 bg-rose-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest"
          >
            Restart Agent
          </button>
        </div>
